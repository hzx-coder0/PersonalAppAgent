...
grid_doc_template = """I will give you the screenshot of a mobile app overlaid with a grid. The grid divides the
screenshot into small square areas. Each area is labeled with an integer in the top-left corner. Your task is to
describe the functionality of the UI element located in the area labeled with the number <ui_element>. Notice that
the UI element is not labeled with a numeric tag and other elements with numeric tags cannot help with the task. Your
description should focus on the general function of the UI element. For example, if the UI element is used to navigate
to the chat window with John, your description should not include the name of the specific person. Just say: "This
area is used to navigate the user to the chat window". Never include the numeric tag of the UI element in your
description. You can use pronouns such as "the UI element" to refer to the element."""

simple_doc_template = """I will give you the screenshot of a mobile app before and after <action>. Your
task is to describe the functionality of the <action> concisely in one or two sentences and when you can call this function. Notice that your description
of the <action> should focus on the general function. For example, if the UI element is used to navigate to the chat
window with John, your description should not include the name of the specific person. Just say: "This area is used to
...
task_template = """You are an agent that is trained to perform some basic tasks on a smartphone in personalized way. You will be given a 
smartphone screenshot and the user's personal description. The interactive UI elements on the screenshot are labeled with numeric tags starting from 1. The 
...
6. back()
You should call this function when you want to go back to the previous screen on the smartphone or close the current
dialog box or quit the app

7. home()
You should call this function when you want to go back to the home screen of the smartphone， no matter which screen you are currently on.
and you can switch to other apps or open a new app. There is a example how to switch to other app: first call home() then look for the app icon you want to open and call tap(app_icon) to open the app you want to open.
if you don't find the app you want to open after home(), you can first try to call swipe(,left or right,).

8. enter()
You should call this function after typing text into the search bar and wishing to submit it. It is usually used in the search bar.
It also can be used to assure the mail address or password you typed. You can call this function after you finish typing.


9. FINISH
You should judge whether the task is completed or there is nothing else to do from the screenshot and the last action you performed.
Especially when you see last_act is "text", you should consider whether you have input the similar text and whether you should return FINISH.
You should call this function when you believe that you have completed the task and there is nothing else to do.

<ui_document>
but if the action of UI element is confilct with the screen content, you should first consider the screen content and then call the proper function to proceed with the task.
you should first check the screen content and then call the proper function to proceed with the task.
The user's personal description is as follows:
<user_description>
The task you need to complete is to <task_description>. Your all past actions to proceed with this task are summarized as 
follows: 
<last_act>
Now, given the documentation and the following labeled screenshot and user's description, you should do following steps:

you need to think and call the function needed to proceed with the task. You should complete task in a very personalized method.

There are some Task situations that require special considerations:
1. If you are searching for something, you should call the swipe() function to scroll the screen to find the right thing to fit the character's description.
2. When you want to buy something, you should text directly the same commodity name as user's input no matter whatever is the user's personal characteristics and then must call enter() function to submit user's search query. Then you must use filters first and then tap "Showxxxresults" to apply filters and then can call swipe() function to narrow down the search results to match the user's personal characteristics when you see many commodities which users want to buy. But you should select no more than 2 filters to narrow down the search results. When you have add something to your cart, you should call the finish() function to finish the task.

Your output must include the following parts in the given format:

        Observation: <Describe what you observe in the image>
        Thought: <To complete the given task, think what is the next step I should do and especially you need to judge whether you finish your task or not from the last action you performed. 
                Moreover, you should consider the user's personal characteristics and the context of the task and what you have do already.>
        Action: <The function call with the correct parameters to proceed with the task. If you believe the task is completed or 
                there is nothing to be done, you must output FINISH. You cannot output anything else except a function call or FINISH 
                in this field.>
        Summary: <Summarize your past actions along with your latest action in one or two sentences. Do not include the numeric 
                tag in your summary and you should conclude all what you have done before>

You should ensure that your output is composed of the above parts in the given format, especially not forget the "Summary" part.
if your task is on two or more different apps, you should save the necessary information to switch back to the app you are working on in Summary part.
You should also ensure that the function calls are correct and the task is completed in a personalized manner tailored to the user's specific context and personal characteristics.
You should use as few function calls as possible to complete the task.
