...
from prompts_test import task_template
...
while round_count < configs["MAX_ROUNDS"]:
    ...
    llast_act.append({"step": round_count, "last_act": last_act})
    llast_act_str = json.dumps(llast_act, ensure_ascii=False)
    print_with_color("before llm round_count: " + str(round_count) +"\tlast_act: " +llast_act_str, "green")
    ...
    if act_name == "tap":
        ...
        content = res[1]
        prompt = re.sub(r"<action>", "inputting text \""+content+"\"", prompts.self_explore_reflect_template)
    elif act_name == "long_press":
    ...
    elif act_name == "home" or act_name == "back" or act_name == "enter":
        prompt = re.sub(r"<action>", act_name, prompts.self_explore_reflect_template)
    else:
        print_with_color("ERROR: Undefined act!", "red")
        break
    #TODO: area在text、home、back、enter时是无效的
    flag = act_name == "home" or act_name == "back" or act_name == "enter"
    flag = not flag
    if act_name == "home" or act_name == "back" or act_name == "enter":
        continue
    # 当action为上面的动作时不做reflect
    else:
