...
while True:
    ...
    if not grid_on: 
        labeled_img = draw_bbox_multi(screenshot_path, os.path.join(labeled_ss_dir, f"{demo_name}_{step}.png"), elem_list,
                                True)
    # else:
    #     rows, cols = draw_grid(screenshot_path, os.path.join(labeled_ss_dir, f"{demo_name}_{step}.png"))
    ...
    print_with_color("Choose one of the following actions you want to perform on the current screen:\ntap, text, long "
                    "press, swipe, grid,stop, back, home, enter", "blue")
    while user_input.lower() != "tap" and user_input.lower() != "text" and user_input.lower() != "long press" \
            and user_input.lower() != "swipe" and user_input.lower() != "stop" and user_input.lower() != "back" \
            and user_input.lower() != "home" and user_input.lower() != "enter" and user_input.lower() != "grid":
        user_input = input()
    if user_input.lower() == "back":
        ...
        if ret == "ERROR":
            ...
            break
        record_file.write("home\n")
    elif user_input.lower() == "enter":
        ret = controller.enter()
        if ret == "ERROR":
            print_with_color("ERROR: enter execution failed", "red")
            break
        record_file.write("enter\n")
    elif user_input.lower() == "grid":
        print_with_color("Grid is not completed yet", "red")
        continue
        # grid_on = True
        # record_file.write("grid\n")
    elif user_input.lower() == "tap":
